...
int main(int argv, char *args[])
{
	...
	auto files = yxp_utility::YXPFileIO::GetDirectoryFiles("I:/My Documents/My Desktop/3DReconstruction/DataSet/lab/image/");


	for (int i=0;i<files.size();++i)
	{
		auto jpgName = yxp_utility::StrAlgo::replaceAll(files[i], ".bmp", ".jpg");
		auto mat = cv::imread(files[i]);
		cv::imwrite(jpgName, mat);
	}
