...
//背景和前景各有一个对应的GMM（混合高斯模型）（model都是1*(13*5)大小的）  
GMM::GMM(Mat& _model)
{
	...
	for (int ci = 0; ci < componentsCount; ci++) //这是初始化的情况下，如果传递给了一个已经有的非空的模型，则要先计算一次协方差矩阵和均值
		if (coefs[ci] > 0)
...
//计算一个像素（由color=（B,G,R）三维double型向量来表示）属于这个GMM混合高斯模型
// (componentsCount个高斯模型分量计算得到的和，其实就是用来判断是属于前景还是背景的模型)的概率。  
//也就是把这个像素像素属于componentsCount个高斯模型的概率与对应的权值相乘再相加，  
//具体见论文的公式（8）。结果从res返回。  
//这个相当于计算Gibbs能量的第一个能量项（取-log后，可以看到后面的constructGraph里面是这样用的）。  
...
//具体过程，即高阶的高斯密度模型计算式，具体见论文的公式（8）。结果从res返回  
double GMM::operator()(int ci, const Vec3d color) const
{
	double res = 0;
	if (coefs[ci] > 0) //coefs是权值
	{
		...
		res = 1.0f / sqrt(covDeterms[ci]) * exp(-0.5f*mult); //这里跟公式里的区别是最后没有除以(pi*2)^(n/2)，因为最终高斯混合模型是去算每个模型的比例，所以公共的部分就不用除了
	}
	return res;
}
...
//GMM参数学习前的初始化，主要是对用来求均值的变量sum和用来求协方差矩阵的prods置零  
...
//从图像数据中(addSample得到的那些数据)学习GMM的参数：每一个高斯分量的权值、均值和协方差矩阵；
...
void GMM::calcInverseCovAndDeterm(int ci)
{
	if (coefs[ci] > 0)
	{
		//这里用的是最原始的求逆矩阵的方法(因为是三阶方阵)，效率也是最高的
		//取第ci个高斯模型的协方差的起始指针  
		double *c = cov + 9 * ci;
		double dtrm =
