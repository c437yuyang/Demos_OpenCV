...
int main1()
{
	Mat image = imread("1.jpg", CV_LOAD_IMAGE_GRAYSCALE); //读为灰度图 
...
void PaddingImage(cv::Mat & srcImage, cv::Mat & dstImage)
{
	...
	{
		imageTemp.create(nWidth + 8 - srcImage.cols % 8, nHeight + 8 - srcImage.rows % 8, 0);//申请
		copyMakeBorder(srcImage, imageTemp, 0, 8 - srcImage.rows % 8, 0, 8 - srcImage.cols % 8, BORDER_CONSTANT, Scalar::all(0));//扩充
	...
	{
		imageTemp.create(nHeight,nWidth + 8 - srcImage.cols % 8 , 0);//申请
		copyMakeBorder(srcImage, imageTemp, 0, 0, 0, 8 - srcImage.cols % 8, BORDER_CONSTANT, Scalar::all(0));//扩充
	...
	{
		imageTemp.create(nWidth, nHeight + 8 - srcImage.rows % 8, 0);//申请
		copyMakeBorder(srcImage, imageTemp, 0, 8 - srcImage.rows % 8, 0, 0, BORDER_CONSTANT, Scalar::all(0));//扩充
